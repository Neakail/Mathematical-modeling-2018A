...
def read_second_layer():
    temp_list = []
    data = xlrd.open_workbook('second_problem_third_layer_1.xls')
    table = data.sheets()[0]
    ncols = table.ncols
    ...
    for second_layer in second_layers:
        # if number < 11:
            ...
            for i in second_layer:
                ...
                temp_list = []
                for each_mm in xrange(1, 51):
                    # 更新温度
                    if second_layer.index(i) == 0:
                        if each_mm == 1:
                            # Q = 0.000045 * 0.1 * (i - list_K[each_mm-1])
                            temp_t = (1 + (each_mm * 1.0 / 50)) * 0.028 * (i - list_K[each_mm - 1]) / (
                            1.18 * 1005 * 0.0001)
                        else:
                            # Q = 0.000045 * 0.1 * (list_K[each_mm-2] - list_K[each_mm-1])
                            temp_t = (1 + (each_mm * 1.0 / 50)) * 0.028 * (
                            list_K[each_mm - 2] - list_K[each_mm - 1]) / (1.18 * 1005 * 0.0001)
        ...
        # number += 1
    write_in_xlwt(all_data, 'second_problem_fourth_layer_2.xls')