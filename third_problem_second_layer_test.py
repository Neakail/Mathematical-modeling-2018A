...
def read_first_layer():
    temp_list = []
    data = xlrd.open_workbook('third_problem_first_layer_test.xls')
    table = data.sheets()[0]
    nrows = table.nrows
...
if __name__ == '__main__':
    ...
    for i in first_layer:
        temp_data = []
        temp_list = []
        for each_mm in xrange(1, 23):
            if first_layer.index(i) == 0:
                if each_mm == 1:
                    # Q = 0.00037 * 0.1 * (i - list_K[each_mm-1])
                    temp_t = (1+(each_mm * 1.0/22)) * 0.37 * (i - list_K[each_mm-1]) / (862 * 2100 * 0.0001)
                else:
                    # Q = 0.00037 * 0.1 * (list_K[each_mm-2] - list_K[each_mm-1])
                    temp_t = (1+(each_mm * 1.0/22)) * 0.37 * (list_K[each_mm-2] - list_K[each_mm-1]) / (862 * 2100 * 0.0001)
        ...
        temp_data[1:23] = list_K
        temp_data.append((temp_data[1]+temp_data[22]) / 2)
        # print temp_data
        all_data.append(temp_data)
    write_in_xlwt(all_data, 'third_problem_second_layer_test.xls')