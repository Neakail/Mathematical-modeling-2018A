...
def read_second_layer():
    temp_list = []
    data = xlrd.open_workbook('third_problem_second_layer_5.xls')
    table = data.sheets()[0]
    ncols = table.ncols
...
if __name__ == '__main__':
    ...
    for second_layer in second_layers:
            ...
            for each_mm in xrange(1, 37):
                ...
                if second_layer.index(i) == 0:
                    if each_mm == 1:
                        ...
                        temp_t = (1 + (each_mm * 1.0 / 36)) * 0.045 * (list_K[each_mm - 2] - list_K[each_mm - 1]) / (
                        74.2 * 1726 * 0.0001)
        ...
        # number += 1
    write_in_xlwt(all_data, 'third_problem_third_layer_5.xls')